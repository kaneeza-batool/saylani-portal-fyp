...
const Hero = () => (
  <section className="relative py-28 lg:py-36 overflow-hidden bg-gradient-to-b from-neutral-50 via-white to-neutral-50/30 text-neutral-900 border-b border-neutral-100">
    ...
    <svg className="absolute inset-0 w-full h-full hidden lg:block pointer-events-none" viewBox="0 0 1200 500" preserveAspectRatio="none">
    ...
    >
      <div className="max-w-4xl mx-auto">
        <motion.span
          variants={fadeInUp}
          className="inline-flex items-center gap-2 px-4 py-1.5 text-xs font-bold mb-8 uppercase te-mono border border-accent-200 bg-accent-50 text-accent-700 rounded-full shadow-sm"
        >
          <span className="w-2 h-2 rounded-full bg-accent-500 animate-pulse" />
          Learn IT Skills &amp; Get Job Placement
        </motion.span>

        <motion.h1 variants={fadeInUp} className="te-display text-4xl sm:text-5xl lg:text-6xl font-extrabold tracking-tight text-primary-900 leading-[1.15]">
        ...
        <motion.p variants={fadeInUp} className="te-body mt-6 text-base sm:text-lg leading-relaxed text-neutral-600 max-w-2xl mx-auto font-normal">
        ...
        <motion.div variants={fadeInUp} className="mt-10 flex flex-wrap justify-center gap-5">
          <motion.div whileHover={{ scale: 1.035 }} whileTap={{ scale: 0.97 }}>
            <Link to="/apply" className="px-8 py-3.5 bg-primary-800 text-white font-bold text-sm rounded-lg hover:bg-primary-700 transition-colors shadow-md shadow-primary-800/10 no-underline block">
              Apply Online Now
            </Link>
          </motion.div>
          <motion.div whileHover={{ scale: 1.035 }} whileTap={{ scale: 0.97 }}>
            <Link to="/programs" className="px-8 py-3.5 border border-neutral-200 bg-white text-neutral-700 font-bold text-sm rounded-lg hover:bg-neutral-50 hover:text-neutral-900 transition-colors no-underline shadow-sm block">
              View All Courses
            </Link>
          </motion.div>
        </motion.div>
...
const StatsBar = () => {
  ...
  return (
    <motion.div {...revealOnScroll} variants={staggerContainer(0.08)} className="border-b border-neutral-100 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 divide-x-0 lg:divide-x divide-neutral-100">
          {stats.map((s, i) => (
            <motion.div key={i} variants={fadeInUp} className="text-center px-4 space-y-2">
    ...
    <section className="py-24 bg-neutral-50/50 border-b border-neutral-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          {points.map((p, idx) => (
            <motion.div key={idx} variants={fadeInUp} whileHover={{ y: -6 }} transition={{ duration: 0.25, ease: 'easeOut' }}>
              <Card className="relative z-10 p-8 h-full bg-white border border-neutral-200/80 rounded-xl shadow-sm hover:border-accent-500 hover:shadow-lg transition-[box-shadow,border-color] duration-300 group">
...
const FeaturedPrograms = () => {
  ...
  return (
    <section className="py-24 bg-white border-b border-neutral-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div {...revealOnScroll} variants={fadeInUp} className="flex flex-col sm:flex-row justify-between items-start sm:items-end mb-16 gap-6">
          <div>
        ...
        <motion.div {...revealOnScroll} variants={staggerContainer(0.12)} className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {tracks.map((t) => (
            <motion.div key={t._id} variants={fadeInUp} whileHover={{ y: -8 }} transition={{ duration: 0.3, ease: 'easeOut' }}>
              <Card className="flex flex-col justify-between h-full bg-white border border-neutral-200 rounded-xl overflow-hidden shadow-sm hover:shadow-2xl hover:border-accent-500 transition-[box-shadow,border-color] duration-300 group">
                <Link to={`/programs/${t.slug}`} className="relative h-48 overflow-hidden bg-neutral-100 block no-underline">
                  {t.img ? (
                    <img src={resolveImageUrl(t.img)} alt={t.title} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
                  ) : (
    ...
    <section className="py-16 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div {...revealOnScroll} variants={fadeInUp} whileHover={{ scale: 1.01 }} transition={{ duration: 0.3, ease: 'easeOut' }}>
          ...
          >
            <motion.div
              className="absolute -right-10 -top-10 w-40 h-40 bg-accent-500/10 rounded-full blur-2xl pointer-events-none"
...
const TimelineSection = () => {
  ...
  return (
    <section className="py-24 bg-primary-900 text-white border-b border-primary-800">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div {...revealOnScroll} variants={fadeInUp}>
          <SectionHeading
        ...
        <motion.div {...revealOnScroll} variants={staggerContainer(0.15)} className="grid grid-cols-1 lg:grid-cols-3 gap-12 relative">
          {steps.map((s, idx) => (
            <motion.div key={idx} variants={fadeInUp}>
              <Card className="relative p-6 border border-primary-800 bg-primary-900/40 rounded-xl h-full hover:border-accent-500/50 transition-colors duration-300">
      ...
      <div className="w-full relative">
        ...
        <div className="animate-marquee-autoplay space-x-6 py-4">
          {infiniteReviews.map((r, i) => (
            <Card key={i} className="w-[380px] shrink-0 p-8 bg-neutral-50 border border-neutral-200/60 rounded-xl shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between">
...
const ROLE_ICONS = {
  ...
  'AI/ML Engineer': (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" /><path d="M9 9h.01M15 9h.01M9 15c1 1 5 1 6 0" /></svg>
  ),
  'Software QA': (
...
const PartnersSection = () => {
  const roles = ['Web Developer', 'Data Analyst', 'AI/ML Engineer', 'Software QA'];
  return (
    <section className="py-16 bg-neutral-50/50 border-b border-neutral-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <motion.p {...revealOnScroll} variants={fadeInUp} className="te-mono text-xs font-bold text-neutral-400 uppercase tracking-widest mb-8">
          The Kind Of Roles Our Graduates Train For
        </motion.p>
        <motion.div {...revealOnScroll} variants={staggerContainer(0.08)} className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {roles.map((role) => (
            <motion.div
              key={role}
              variants={fadeInUp}
              whileHover={{ y: -3 }}
              className="flex items-center justify-center gap-2.5 px-4 py-4 bg-white border border-neutral-200 rounded-xl text-neutral-700 shadow-sm hover:border-accent-300 hover:shadow-md transition-[box-shadow,border-color] duration-300"
            >
              <span className="text-accent-500 shrink-0">{ROLE_ICONS[role]}</span>
              <span className="font-extrabold te-display text-sm sm:text-base tracking-wide">{role}</span>
            </motion.div>
          ))}
        </motion.div>
      </div>
  ...
  return (
    <section className="py-24 bg-white border-b border-neutral-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        <motion.div {...revealOnScroll} variants={staggerContainer(0.1)} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mt-16 text-left">
          {hubs.map((h, idx) => (
            <motion.div key={idx} variants={fadeInUp} whileHover={{ y: -6 }} transition={{ duration: 0.25, ease: 'easeOut' }}>
              <Card className="p-8 h-full bg-neutral-50/50 border border-neutral-200 rounded-xl shadow-sm hover:bg-white hover:shadow-lg transition-[box-shadow,background-color] duration-300">
  ...
  return (
    <section className="py-24 bg-neutral-50/50 border-b border-neutral-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <motion.div {...revealOnScroll} variants={staggerContainer(0.12)} className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <motion.div variants={fadeInUp}>
            <Card className="p-8 h-full border border-neutral-200 rounded-2xl bg-white shadow-sm hover:border-accent-500 hover:shadow-lg transition-[box-shadow,border-color] duration-300">
  ...
  return (
    <section className="py-24 bg-white border-b border-neutral-100">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <motion.div {...revealOnScroll} variants={staggerContainer(0.06)} className="space-y-4">
          {faqs.map((f, idx) => {
            const isOpen = openIdx === idx;
            return (
              <motion.div key={idx} variants={fadeInUp} className="border border-neutral-200 rounded-xl overflow-hidden bg-neutral-50/50">
                <button onClick={() => setOpenIdx(isOpen ? null : idx)} className="w-full p-6 text-left font-bold text-neutral-900 flex justify-between items-center te-display text-base transition-colors hover:bg-neutral-100/50">
                  <span>{f.q}</span>
                  <motion.span animate={{ rotate: isOpen ? 45 : 0 }} transition={{ duration: 0.2 }} className="text-accent-600 font-mono text-xl shrink-0 ml-4">
                    +
                  </motion.span>
                </button>
...
const BottomCTA = () => {
  return (
    <section className="py-24 bg-neutral-50 text-center">
      <motion.div {...revealOnScroll} variants={fadeInUp} className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="mt-10 flex justify-center gap-5">
          <motion.div whileHover={{ scale: 1.035 }} whileTap={{ scale: 0.97 }}>
            <Link to="/apply" className="px-8 py-3.5 bg-primary-800 text-white font-bold text-sm rounded-lg hover:bg-primary-700 transition-colors shadow-md no-underline block">
              Apply Online Now
            </Link>
          </motion.div>
          <motion.div whileHover={{ scale: 1.035 }} whileTap={{ scale: 0.97 }}>
            <Link to="/contact" className="px-8 py-3.5 border border-neutral-300 bg-white text-neutral-800 font-bold text-sm rounded-lg hover:bg-neutral-50 no-underline block">
