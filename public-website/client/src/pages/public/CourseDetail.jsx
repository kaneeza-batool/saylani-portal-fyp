...
const Hero = ({ track }) => (
  <section className="relative py-16 lg:py-20 overflow-hidden bg-gradient-to-b from-neutral-50 via-white to-neutral-50/30 text-neutral-900 border-b border-neutral-100">
    ...
    <motion.div variants={staggerContainer(0.1)} initial="hidden" animate="show" className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
      <motion.nav variants={fadeInUp} className="te-mono text-[11px] font-bold text-neutral-400 uppercase tracking-wider mb-6" aria-label="Breadcrumb">
      ...
      <motion.h1 variants={fadeInUp} className="te-display text-3xl sm:text-4xl lg:text-5xl font-extrabold tracking-tight text-primary-900 leading-[1.15]">
        {track.title}
      </motion.h1>
      <motion.p variants={fadeInUp} className="te-body mt-5 text-base sm:text-lg leading-relaxed text-neutral-600 max-w-3xl font-normal">
        {track.tagline}
      </motion.p>
      ...
      <motion.div variants={fadeInUp} className="mt-8 flex flex-wrap items-center gap-4">
        <motion.div whileHover={{ scale: 1.035 }} whileTap={{ scale: 0.97 }}>
          <Link
            ...
            className="block px-7 py-3.5 font-bold text-sm rounded-lg text-center no-underline transition-colors shadow-md bg-primary-900 text-white hover:bg-primary-800 shadow-primary-900/15"
          >
            Enroll Now →
          </Link>
        </motion.div>
...
// --- WHAT YOU'LL LEARN — connected month-by-month timeline ---
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
      <motion.div {...revealOnScroll} variants={fadeInUp}>
        <SectionHeading
          ...
          description="A month-by-month roadmap — each stage builds directly on the last."
          descriptionClassName="te-body mt-2 text-neutral-500 text-sm"
        />
      </motion.div>

      <motion.div {...revealOnScroll} variants={staggerContainer(0.15)} className="relative">
        {/* Connecting line — draws downward as the section scrolls into view */}
        <motion.div
          variants={{ hidden: { scaleY: 0 }, show: { scaleY: 1, transition: { duration: 1.1, ease: [0.65, 0, 0.35, 1] } } }}
          style={{ originY: 0 }}
          className="hidden sm:block absolute left-5 top-1 bottom-1 w-px bg-gradient-to-b from-accent-400 via-neutral-200 to-neutral-100"
        />

        <div className="space-y-6">
          {track.curriculum.map((month, idx) => (
            <motion.div key={idx} variants={fadeInUp} className="relative sm:pl-16">
              <div className="hidden sm:flex absolute left-0 top-0 h-10 w-10 rounded-full bg-primary-900 text-white items-center justify-center font-bold te-mono text-sm shadow-md shadow-primary-900/20 z-10 ring-4 ring-white">
                {month.monthNumber}
              </div>
              <motion.div whileHover={{ y: -3 }} transition={{ duration: 0.2 }}>
                <Card className="p-6 bg-neutral-50/40 border border-neutral-100 rounded-xl hover:border-accent-300 hover:shadow-md transition-[box-shadow,border-color] duration-300">
                  <h3 className="te-display text-sm font-bold text-neutral-900 mb-3">
                    <span className="sm:hidden">Month {month.monthNumber}: </span>
                    {month.title}
                  </h3>
...
const ToolsSection = ({ track }) => (
  <section className="py-20 bg-neutral-50/50 border-b border-neutral-100">
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <motion.div {...revealOnScroll} variants={staggerContainer(0.04)} className="flex flex-wrap gap-3">
        {track.languages.map((lang, idx) => (
          <motion.div key={`lang-${idx}`} variants={fadeInUp} whileHover={{ y: -2, scale: 1.04 }}>
            <Badge className="te-mono text-xs font-bold text-white bg-primary-900 px-4 py-2 rounded-lg block">
              {lang}
            </Badge>
          </motion.div>
        ))}
        {track.tools.map((tool, idx) => (
          <motion.div key={`tool-${idx}`} variants={fadeInUp} whileHover={{ y: -2, scale: 1.04 }}>
            <Badge className="te-mono text-xs font-bold text-neutral-900 bg-white border border-neutral-200 px-4 py-2 rounded-lg shadow-xs block hover:border-accent-300 transition-colors">
              {tool}
            </Badge>
          </motion.div>
        ))}
...
const WhoForSection = ({ track }) => (
  <section className="py-20 bg-white border-b border-neutral-100">
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
      <motion.div {...revealOnScroll} variants={staggerContainer(0.15)} className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-start">
...
const CareerOutcomesSection = ({ track }) => (
  <section className="py-20 bg-primary-900 text-white border-b border-primary-800">
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
      ...
      <motion.div {...revealOnScroll} variants={staggerContainer(0.08)} className="mt-10 flex flex-wrap items-center justify-center gap-4">
        {track.careerOutcomes.map((role, idx) => (
          <motion.div
            key={idx}
            variants={fadeInUp}
            whileHover={{ y: -3, backgroundColor: 'rgba(255,255,255,0.09)' }}
            className="px-6 py-4 bg-white/5 border border-white/10 rounded-xl backdrop-blur-sm transition-colors"
          >
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div {...revealOnScroll} variants={fadeInUp}>
          <SectionHeading
            ...
            titleClassName="te-display text-2xl sm:text-3xl font-bold text-neutral-900 mt-3"
          />
          <Card className="p-8 bg-white border border-neutral-200/80 rounded-xl shadow-sm hover:shadow-lg transition-shadow duration-300">
  ...
  return (
    <section className="py-20 bg-white border-b border-neutral-100">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <motion.div {...revealOnScroll} variants={staggerContainer(0.1)} className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {related.map((r) => (
            <motion.div key={r._id} variants={fadeInUp} whileHover={{ y: -6 }} transition={{ duration: 0.25, ease: 'easeOut' }}>
              <Link
                to={`/programs/${r.slug}`}
                className="block h-full p-6 bg-neutral-50/40 border border-neutral-100 rounded-xl hover:border-primary-600 hover:shadow-lg transition-[box-shadow,border-color] duration-300 no-underline group"
              >
                ...
                <span className="te-mono text-[10px] font-bold text-accent-600 mt-4 inline-flex items-center gap-1 group-hover:gap-2 transition-all">View Details →</span>
              </Link>
            </motion.div>
          ))}
        </motion.div>
...
const BottomCTA = ({ track }) => (
  <section className="py-20 bg-neutral-50 text-center">
    <motion.div {...revealOnScroll} variants={fadeInUp} className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <motion.div className="mt-8 inline-block" whileHover={{ scale: 1.035 }} whileTap={{ scale: 0.97 }}>
        <Link
          to={`/apply?program=${track._id}`}
          className="inline-block px-8 py-3.5 font-bold text-sm rounded-lg no-underline transition-colors shadow-md bg-primary-800 text-white hover:bg-primary-700 shadow-primary-900/15"
        >
          Enroll Now
        </Link>
      </motion.div>
    ...
    <AnimatePresence>
      {visible && (
        <motion.div
          ...
          className="fixed bottom-0 left-0 right-0 z-40 bg-white/90 backdrop-blur-xl border-t border-neutral-200 shadow-[0_-8px_30px_rgba(0,0,0,0.06)]"
        >
