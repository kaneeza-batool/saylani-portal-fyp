...
import { checkApplicationStatus } from '../../services/publicJobService';
...
const STATUS_STYLE = {
  ...
  interview_scheduled: { label: 'Interview Scheduled', className: 'bg-info-bg text-info-text' },
...
export default function StatusCheckPage() {
  ...
  const mutation = useMutation({ mutationFn: () => checkApplicationStatus(email.trim()) });
  ...
  return (
    <motion.div variants={fadeIn} initial="hidden" animate="show" className="flex flex-col gap-5 max-w-[560px]">
      <div>
        <h1 className="font-heading font-bold text-h4 text-navy-900">Check Application Status</h1>
        <p className="text-body-sm text-neutral-500 mt-1">Enter the email address you applied with to see the status of your application(s).</p>
      </div>
        ...
        <motion.div variants={fadeIn} initial="hidden" animate="show" className="flex flex-col gap-3">
          {mutation.data.length === 0 ? (
            <div className="bg-surface border border-neutral-200 rounded-xl p-6 text-center text-body-sm text-neutral-400">
              No applications found for that email.
            </div>
          ) : (
            mutation.data.map((app, i) => {
              ...
              return (
                <div key={i} className="bg-surface border border-neutral-200 rounded-xl p-5 flex items-center justify-between gap-3 flex-wrap">
                  <div>
                    <div className="font-heading font-bold text-body text-neutral-900">{app.jobTitle}</div>
                    <div className="text-caption text-neutral-400">Applied {fmtDate(app.createdAt)}</div>
                  </div>
