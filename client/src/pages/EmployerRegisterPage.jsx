...
export default function EmployerRegisterPage() {
  const { user, loading, registerEmployer: submitRegistration } = useAuth();
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      await submitRegistration({ companyName, contactEmail, contactPhone, city, password });
    ...
    <div className="relative min-h-screen w-full flex items-center justify-center bg-navy-900 font-sans px-4 overflow-hidden py-10">
      ...
      >
        <div className="flex flex-col items-center gap-2">
          ...
          <div className="text-caption text-navy-300 font-normal mt-1">Employer Portal — Create Account</div>
        </div>

        <form onSubmit={handleSubmit} className="flex flex-col gap-3.5">
          <div className="flex flex-col gap-1.5">
            <label htmlFor="companyName" className={labelClass}>Company Name</label>
            <input id="companyName" required value={companyName} onChange={(e) => setCompanyName(e.target.value)} placeholder="Your company's name" className={inputClass} />
          </div>

          <div className="flex flex-col gap-1.5">
            <label htmlFor="contactEmail" className={labelClass}>Contact Email</label>
            <input id="contactEmail" type="email" autoComplete="email" required value={contactEmail} onChange={(e) => setContactEmail(e.target.value)} placeholder="hr@yourcompany.com" className={inputClass} />
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div className="flex flex-col gap-1.5">
              <label htmlFor="contactPhone" className={labelClass}>Phone</label>
              <input id="contactPhone" value={contactPhone} onChange={(e) => setContactPhone(e.target.value)} placeholder="0300-1234567" className={inputClass} />
            </div>
            <div className="flex flex-col gap-1.5">
              <label htmlFor="city" className={labelClass}>City</label>
              <input id="city" value={city} onChange={(e) => setCity(e.target.value)} placeholder="e.g. Karachi" className={inputClass} />
            </div>
          </div>
          ...
          <p className="text-micro text-navy-300 leading-relaxed">
            A TITAN admin reviews and verifies new employer accounts before your job postings can go public — you can still draft jobs while you wait.
          </p>
