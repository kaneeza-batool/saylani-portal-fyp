...
const STATUS_STYLE = {
  ...
  Live: { label: 'Live', className: 'bg-info-bg text-info-text' },
...
export default function QuizzesPage() {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ['quizzes', { search, status, page }],
    queryFn: () => fetchQuizzes({ search, status, page, limit: 8 }),
  ...
  const createMutation = useMutation({
    ...
    onError: (err) => setFormError(err.response?.data?.message || 'Failed to add quiz.'),
  });
    ...
    <motion.div variants={staggerContainer} initial="hidden" animate="show" className="flex flex-col gap-4">
      ...
      <motion.div variants={fadeInUp} className="bg-white border border-neutral-200 rounded-xl overflow-hidden">
        ...
        {isLoading ? (
          ...
          <div className="py-14 px-5 text-center text-danger-600 text-body-sm">Couldn't load quizzes. Please try again.</div>
        ) : items.length === 0 ? (
          <div className="py-14 px-5 text-center text-neutral-400 text-body-sm">No quizzes match this search.</div>
        ) : (
          <motion.div variants={staggerContainer} initial="hidden" animate="show">
            {items.map((q) => {
              const s = STATUS_STYLE[q.status] ?? STATUS_STYLE.Scheduled;
              return (
                <motion.div
                  key={q._id}
                  variants={fadeInUp}
                ...
                >
                  ...
                  <span className="text-body-sm text-neutral-600 truncate">{q.campus}</span>
                  <span className="text-body-sm text-neutral-600">{q.attempts}</span>
                  <span className="text-body-sm text-neutral-600">{q.avg}%</span>
      ...
      {!isLoading && !isError && total > 0 && (
        <div className="flex items-center justify-between text-body-sm text-neutral-400">
          <span>
            {total} quiz{total === 1 ? '' : 'zes'} · Page {page} of {pages}
          </span>
