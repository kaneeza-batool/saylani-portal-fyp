...
const GRID_COLS = 'grid-cols-[1.5fr_1fr_1.6fr_1fr_0.8fr_0.8fr]';
...
export default function CampusesPage() {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ['campuses', { search, status, page }],
    queryFn: () => fetchCampuses({ search, status, page, limit: 8 }),
  ...
  const createMutation = useMutation({
    ...
    onError: (err) => setFormError(err.response?.data?.message || 'Failed to add campus.'),
  });

  const updateMutation = useMutation({
    mutationFn: ({ id, payload }) => updateCampus(id, payload),
    onSuccess: () => {
      invalidate();
      setModal({ open: false, mode: 'add', item: null });
      setFormError('');
    },
    onError: (err) => setFormError(err.response?.data?.message || 'Failed to update campus.'),
  });

  const deleteMutation = useMutation({ mutationFn: deleteCampus, onSuccess: invalidate });
    ...
    <motion.div variants={staggerContainer} initial="hidden" animate="show" className="flex flex-col gap-4">
      ...
      <motion.div variants={fadeInUp} className="bg-white border border-neutral-200 rounded-xl overflow-hidden">
        ...
        {isLoading ? (
          ...
          <div className="py-14 px-5 text-center text-danger-600 text-body-sm">Couldn't load campuses. Please try again.</div>
        ) : items.length === 0 ? (
          <div className="py-14 px-5 text-center text-neutral-400 text-body-sm">No campuses match this search.</div>
        ) : (
            ...
            {items.map((c) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="text-body-sm text-neutral-600 truncate">{c.address || '—'}</span>
                  <span className="text-body-sm text-neutral-600">{c.phone || '—'}</span>
      ...
      {!isLoading && !isError && total > 0 && (
        <div className="flex items-center justify-between text-body-sm text-neutral-400">
          <span>
            {total} campus{total === 1 ? '' : 'es'} · Page {page} of {pages}
          </span>
