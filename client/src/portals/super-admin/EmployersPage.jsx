...
export default function EmployersPage() {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ['employers', { search, status, page }],
    queryFn: () => fetchEmployers({ search, status, page, limit: 8 }),
    keepPreviousData: true,
  });

  const invalidate = () => queryClient.invalidateQueries({ queryKey: ['employers'] });
  ...
  const createMutation = useMutation({
    ...
    onError: (err) => setFormError(err.response?.data?.message || 'Failed to add employer.'),
  });

  const updateMutation = useMutation({
    mutationFn: ({ id, payload }) => updateEmployer(id, payload),
    onSuccess: () => {
      invalidate();
      setModal({ open: false, mode: 'add', item: null });
      setFormError('');
    },
    onError: (err) => setFormError(err.response?.data?.message || 'Failed to update employer.'),
  });

  const deleteMutation = useMutation({ mutationFn: deleteEmployer, onSuccess: invalidate });
  ...
  const handleDelete = (item) => {
    if (window.confirm(`Delete ${item.companyName}? This can't be undone.`)) deleteMutation.mutate(item._id);
  };
    ...
    <motion.div variants={staggerContainer} initial="hidden" animate="show" className="flex flex-col gap-4">
      ...
      <motion.div variants={fadeInUp} className="bg-white border border-neutral-200 rounded-xl overflow-hidden">
        ...
        {isLoading ? (
          ...
          <div className="py-14 px-5 text-center text-danger-600 text-body-sm">Couldn't load employers. Please try again.</div>
        ) : items.length === 0 ? (
          <div className="py-14 px-5 text-center text-neutral-400 text-body-sm">No employers match this search.</div>
        ) : (
            ...
            {items.map((e) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="text-body-sm text-neutral-600">{e.contactPhone || '—'}</span>
                  <span className="text-body-sm text-neutral-600">{e.city || '—'}</span>
      ...
      {!isLoading && !isError && total > 0 && (
        <div className="flex items-center justify-between text-body-sm text-neutral-400">
          <span>
            {total} employer{total === 1 ? '' : 's'} · Page {page} of {pages}
          </span>
