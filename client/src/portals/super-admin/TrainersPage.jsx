...
export default function TrainersPage() {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ['trainers', { search, status, page }],
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, payload }) => updateTrainer(id, payload),
    onSuccess: () => {
      invalidate();
      setModal({ open: false, mode: 'add', item: null });
      setFormError('');
  ...
  return (
    <motion.div variants={staggerContainer} initial="hidden" animate="show" className="flex flex-col gap-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2.5">
          <ExportButtons title="Trainers" filenameBase="titan-trainers" columns={EXPORT_COLUMNS} rows={items} />
      ...
      <motion.div variants={fadeInUp} className="bg-surface border border-neutral-200 rounded-xl overflow-x-auto">
        ...
        {isLoading ? (
          ...
          <div className="py-14 px-5 text-center text-danger-600 text-body-sm">Couldn't load trainers. Please try again.</div>
        ) : items.length === 0 ? (
