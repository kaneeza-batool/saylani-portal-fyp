...
export default function StudentsPage() {
  ...
  const { data, isLoading, isFetching, isError } = useQuery({
    ...
    queryFn: () => fetchStudents({ search, status, page, limit: 8 }),
  ...
  const createMutation = useMutation({
    ...
    onError: (err) => setFormError(err.response?.data?.message || 'Failed to add student.'),
  });
  ...
  const handleSubmit = (values) => {
    if (modal.mode === 'add') {
      ...
      updateMutation.mutate({ id: modal.student._id, payload: values });
    }
  };
    ...
    <motion.div variants={staggerContainer} initial="hidden" animate="show" className="flex flex-col gap-4">
      ...
      <motion.div variants={fadeInUp} className="bg-surface border border-neutral-200 rounded-xl overflow-x-auto">
        ...
        {isLoading ? (
          ...
          <div className="py-14 px-5 text-center text-danger-600 text-body-sm">Couldn't load students. Please try again.</div>
        ) : students.length === 0 ? (
