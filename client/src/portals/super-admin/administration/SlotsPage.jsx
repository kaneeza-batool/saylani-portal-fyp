...
const EXPORT_COLUMNS = [
  { header: 'Schedule', accessor: (s) => s.schedule },
  { header: 'Trainer', accessor: (s) => s.trainer },
  { header: 'Course', accessor: (s) => s.course },
  { header: 'Campus', accessor: (s) => s.campus?.name },
  { header: 'Seats Filled', accessor: (s) => s.seatsFilled },
  { header: 'Seats Total', accessor: (s) => s.seatsTotal },
...
function RowSkeleton() {
  return (
    <div className={`grid ${GRID_COLS} min-w-[900px] gap-[16px] px-[18px] py-3.5 items-center border-b border-neutral-100`}>
  ...
  return (
    <motion.div variants={staggerContainer} initial="hidden" animate="show" className="flex flex-col gap-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2.5">
          <ExportButtons title="Slots" filenameBase="titan-slots" columns={EXPORT_COLUMNS} rows={items} />
      ...
      <motion.div variants={fadeInUp} className="bg-surface border border-neutral-200 rounded-xl overflow-x-auto">
        <div className={`grid ${GRID_COLS} min-w-[900px] gap-[16px] px-[18px] py-3.5 bg-neutral-50 border-b border-neutral-200`}>
            ...
            {items.map((s) => {
              ...
              return (
                <motion.div
                  key={s._id}
                  variants={fadeInUp}
                  className={`grid ${GRID_COLS} min-w-[900px] gap-[16px] px-[18px] py-3.5 items-center border-b border-neutral-100 last:border-b-0 transition-colors hover:bg-neutral-50`}
                >
      ...
      <ConfirmDialog
        open={!!deleteTarget}
        title="Delete slot"
        message={deleteTarget ? `Delete this slot (${deleteTarget.course})? This can't be undone.` : ''}
